window.addEventListener("load", function(){
    ...
    let botonTexto = document.createTextNode("Cambiar modo");
    botonModo.appendChild(botonTexto);
    this.document.body.appendChild(botonModo);
    botonModo.classList.add("botonModo");
...
function crearNoticia(urlImage, titulo, contenido){
    const template=`
    <div class="item">
      <img src="${urlImage}">
      <h2>${titulo}</h2>
...
let noticias=[
    {
        ...
        pertenecientes al género Panthera. Se encuentra solamente en el continente asiático; es un predador carnívoro y es la especie de félido más grande del mundo junto con el león pudiendo alcanzar ambos un tamaño comparable al de
...
function insertarNoticias(array){
    for (let i = 0; i < array.length; i++) {
        crearNoticia(array[i].urlImage, array[i].titulo, array[i].contenido);
    }
}

insertarNoticias(noticias);
